...
RUN apt-get update && \
    apt-get install -y prometheus prometheus-node-exporter curl libfontconfig1 musl && \
    apt-get clean

# 2. Install Grafana (Direct Download)
...
# We added 'prometheus-node-exporter' to the startup list
RUN echo '#!/bin/bash' > /start.sh && \
    echo 'echo "Starting Node Exporter..."' >> /start.sh && \
    echo 'prometheus-node-exporter &' >> /start.sh && \
